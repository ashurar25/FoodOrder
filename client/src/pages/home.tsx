...
export default function Home() {
  ...
  const { data: initData } = useQuery({
    queryKey: ["/api/init"],
    queryFn: () => apiRequest("POST", "/api/init"),
    staleTime: Infinity, // Only run once
  });

  const { data: restaurant } = useQuery<Restaurant>({
    queryKey: ["/api/restaurant"],
    queryFn: () => apiRequest("GET", "/api/restaurant"),
    enabled: !!initData, // Wait for init to complete
  });
  ...
  const { data: banners = [] } = useQuery<Banner[]>({
    queryKey: ["/api/banners"],
    queryFn: () => apiRequest("GET", "/api/banners"),
    enabled: !!initData, // Wait for init to complete
  });

  const { data: foodItems = [], isLoading: foodItemsLoading } = useQuery<FoodItem[]>({
    queryKey: ["/api/food-items", selectedCategory, searchQuery],
    queryFn: async () => {
      if (searchQuery.trim()) {
        return apiRequest("GET", `/api/food-items/search?q=${encodeURIComponent(searchQuery)}`);
      }
      const params = selectedCategory ? `?categoryId=${selectedCategory}` : "";
      return apiRequest("GET", `/api/food-items${params}`);
    },
    enabled: !!initData, // Wait for init to complete
  });
